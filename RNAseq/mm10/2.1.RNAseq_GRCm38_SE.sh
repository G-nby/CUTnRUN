...
if [ "$is_trim" = "trim" ]; then
	...
	rm ${file_prefix}_val*.fq
        ...
        # mapping
        echo "Start mapping"
        hisat2 -p 8 --dta -x /Data/lht/lilab_users/25.GBY/Ensembl_genome/GRCm38/hisat2_index/GRCm38 -U ${file_prefix}.f*q -S ${file_prefix}.sam 2>>${file_prefix}_log.txt &
        wait
        echo "Mapping done"
fi
...
htseq-count -s no -r pos -t exon -n 16 -i gene_id -f bam ${file_prefix}_sorted.bam /Data/lht/lilab_users/25.GBY/Ensembl_genome/GRCm38/Mus_musculus.GRCm38.102.gtf   > ${file_prefix}_Mm_GRCm38.109.chr.counts &
wait
