...
if [ "$is_trim" = "trim" ]; then
	...
	# mapping
	echo "Start mapping"
	hisat2 -p 8 --dta -x /Data/lht/lilab_users/25.GBY/Ensembl_genome/GRCm38/hisat2_index/GRCm38 -1 ${file_prefix}_1*val*.fq -2 ${file_prefix}_2*val*.fq -S ${file_prefix}.sam 2>>${file_prefix}_log.txt &
	wait
	echo "Mapping done"
	...
	# mapping
	echo "Start mapping"
	hisat2 -p 8 --dta -x /Data/lht/lilab_users/25.GBY/Ensembl_genome/GRCm38/hisat2_index/GRCm38 -1 ${file_prefix}_1*f*q.gz -2 ${file_prefix}_2*f*q.gz -S ${file_prefix}.sam 2>>${file_prefix}_log.txt &
	wait
	echo "Mapping done"
fi
