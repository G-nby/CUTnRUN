...
if [ $count == 1 ]; then
        ...
        sh /Share/home/gby/CUTnRUN/RNAseq/mm10/2.1.RNAseq_GRCm38_SE.sh ${file_prefix} $1
elif [ $count == 2 ]; then
        i=`ls *_1*f*q`
        file_prefix=${i%_1*f*q}
        echo "File prefix=${file_prefix}"
        echo "Two sequencing data files detected, paired-end mode applied."
        sh /Share/home/gby/CUTnRUN/RNAseq/mm10/2.2.RNAseq_GRCm38_PE.sh ${file_prefix} $1
else
