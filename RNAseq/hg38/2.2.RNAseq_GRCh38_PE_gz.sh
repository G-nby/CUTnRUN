...
if [ "$is_trim" = "trim" ]; then
	...
	# trim
	echo 'Start trimming'
	trim_galore --paired ${file_prefix}_1*f*q.gz ${file_prefix}_2*f*q.gz --dont_gzip
	echo "Trimming done"

	# mapping
	echo "Start mapping"
	hisat2 -p 8 --dta -x /Share/home/lht/bioinformatic_tools/Ensembl_genome/GRCh38/hisat2_index/GRCh38 -1 ${file_prefix}_1*val*.fq -2 ${file_prefix}_2*val*.fq -S ${file_prefix}.sam 2>>${file_prefix}_log.txt &
	wait
	echo "Mapping done"
	...
	# mapping
	echo "Start mapping"
	hisat2 -p 8 --dta -x /Share/home/lht/bioinformatic_tools/Ensembl_genome/GRCh38/hisat2_index/GRCh38 -1 ${file_prefix}_1*f*q.gz -2 ${file_prefix}_2*f*q.gz -S ${file_prefix}.sam 2>>${file_prefix}_log.txt &
	wait
	echo "Mapping done"
fi
