...
if [ "$is_trim" = "trim" ]; then
	...
	rm ${file_prefix}_val*.fq
        ...
        # mapping
        echo "Start mapping"
        hisat2 -p 8 --dta -x /Share/home/lht/bioinformatic_tools/Ensembl_genome/GRCh38/hisat2_index/GRCh38 -U ${file_prefix}.f*q -S ${file_prefix}.sam 2>>${file_prefix}_log.txt &
        wait
        echo "Mapping done"
fi
