...
ecoli_bowtie2_index='/Data/lht/lilab_users/25.GBY/Ensembl_genome/ecoli/Escherichia_coli_K_12_DH10B/Ensembl/EB1/Sequence/Bowtie2Index'
...
if [ $is_trim == "trim" ] && [ ${SEorPE} == "PE" ]; then
        ...
        echo "Start mapping to ecoli"
        bowtie2 -p 16 -t -q  -N 1 -L 25 -X 700 --no-unal -x ${ecoli_bowtie2_index} -1 ${file_prefix}_1*_val_1.fq -2 ${file_prefix}_2*_val_2.fq > ecoli_${file_prefix}.sam 2>>ecoli_${file_prefix}_log.txt
        wait
        ...
        echo 'Mode: No trimming + PE,mapping to ecoli'
        bowtie2 -p 16 -t -q  -N 1 -L 25 -X 700 --no-unal -x ${ecoli_bowtie2_index} -1 ${file_prefix}_1*.f*q -2 ${file_prefix}_2*.f*q > ecoli_${file_prefix}.sam 2>>ecoli_${file_prefix}_log.txt
        wait
        ...
        # Mapping to ecoli
        echo "mapping to ecoli"
        bowtie2 -p 16 -t -q  -N 1 -L 25 -X 700 --no-unal -x ${ecoli_bowtie2_index} -U ${file_prefix}_trimmed.fq > ecoli_${file_prefix}.sam 2>>ecoli_${file_prefix}_log.txt
        wait
        ...
        bowtie2 -p 16 -t -q  -N 1 -L 25 -X 700 --no-unal -x ${ecoli_bowtie2_index} -U ${file_prefix}.f*q > ecoli_${file_prefix}.sam 2>>ecoli_${file_prefix}_log.txt
