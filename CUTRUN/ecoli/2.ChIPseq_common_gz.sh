...
ecoli_bowtie2_index='/Data/lht/lilab_users/25.GBY/Ensembl_genome/ecoli/Escherichia_coli_K_12_DH10B/Ensembl/EB1/Sequence/Bowtie2Index/genome'
...
if [ $is_trim == "trim" ] && [ ${SEorPE} == "PE" ]; then
        ...
        echo "bowtie2 -p 16 -t -q  -N 1 -L 25 -X 700 --no-unal -x $ecoli_bowtie2_index -1 $file_prefix_1*.f*q.gz -2 $file_prefix_2*.f*q.gz > ecoli_$file_prefix.sam 2>>ecoli_${file_prefix}_log.txt"
        bowtie2 -p 16 -t -q  -N 1 -L 25 -X 700 --no-unal -x ${ecoli_bowtie2_index} -1 ${file_prefix}_1*.f*q.gz -2 ${file_prefix}_2*.f*q.gz > ecoli_${file_prefix}.sam 2>>ecoli_${file_prefix}_log.txt
        wait
        ...
        bowtie2 -p 16 -t -q  -N 1 -L 25 -X 700 --no-unal -x ${ecoli_bowtie2_index} -U ${file_prefix}.f*q.gz > ecoli_${file_prefix}.sam 2>>ecoli_${file_prefix}_log.txt
