...
# ================= 手动配置区 =================
# 1. 路径设置
input_rds_path <- "/Work/lht/lilab_users/25.GBY/peak_cor_test/zwj/0513/merged_plot_data.rds" # 替换为你的真实路径
cache_files <- list(plot_data = input_rds_path)
figures_dir <- "/Work/lht/lilab_users/25.GBY/peak_cor_test/zwj/0513/results/figures1"
output_dir <- figures_dir

# 2. 分析目标设置
# 定义你要对比的样本对 (必须是 sample 列中存在的名字)
SPECIFIC_PLOT_PAIRS <- list(
  c("F296P_HDAC2_DIR3", "F296P_MTA1p_DIR2"),
  c("GFP_HDAC2_DIR3", "GFP_MTA1p_DIR2"),
  c("MTA1_HDAC2_DIR3", "MTA1_MTA1p_DIR2"),
  c("Y13A_HDAC2_DIR3", "Y13A_MTA1p_DIR2"),
  c("singleP_HDAC2_DIR3", "singleP_MTA1p_DIR2")
)

# 定义 Boxplot 和全矩阵热图中的样本顺序
SAMPLES_FOR_GENE_BOXPLOT <- c("F296P_MTA1p_DIR2", "GFP_MTA1p_DIR2", "MTA1_MTA1p_DIR2", "Y13A_MTA1p_DIR2", "singleP_MTA1p_DIR2")

# 3. 标签数量设置
N_LABELS_TE_FAMILY <- 10
N_LABELS_TE_REPNAME <- 15
N_LABELS_GENES <- 20

# 定义感兴趣的 TE ，占位变量，应该不用改
TE_CLASSES_OF_INTEREST <- c("LTR", "DNA", "SINE", "LINE") 
TE_repname_OI <- c("L1Md_T", "IAPEY3-int") 

plot_data <- readRDS(input_rds_path)


# =============================================
# --- 处理mta1的除以的问题 ---
message("--> [正在计算] 样本对之间的数值比例 (Ratio Calculation)...")

new_ratio_samples <- list()

for (pair in SPECIFIC_PLOT_PAIRS) {
  x_name <- pair[1] # HDAC2 相关
  y_name <- pair[2] # MTA1p 相关
  new_sample_name <- paste0(x_name, "_div_", y_name)
  
  message(paste("    计算比例:", new_sample_name))
  
  # 提取这对样本的数据
  pair_data <- plot_data %>%
    filter(sample %in% c(x_name, y_name))
    
  # 转换为宽表以对齐 geneID/repName
  # 我们需要保留所有的元数据列 (featureType, Class, Family, milliDiv 等)
  # 所以在 pivot_wider 的 id_cols 中包含它们
  ratio_df <- pair_data %>%
    select(geneID, sample, lfc_over_igg, featureType, Class, Family, repName, milliDiv) %>%
    pivot_wider(
      id_cols = c(geneID, featureType, Class, Family, repName, milliDiv),
      names_from = sample,
      values_from = lfc_over_igg
    ) %>%
    # 执行除法计算。注意：如果分母为0，结果会产生 Inf，这里可以根据需要处理
    #log空间，所以是相减
    mutate(
      lfc_over_igg = .data[[x_name]] - .data[[y_name]],
      sample = new_sample_name
    ) %>%
    # 移除原始的列，保持与 plot_data 格式一致
    select(geneID, sample, lfc_over_igg, featureType, Class, Family, repName, milliDiv) %>%
    filter(!is.na(lfc_over_igg)) # 移除无法计算的行
    
  new_ratio_samples[[length(new_ratio_samples) + 1]] <- ratio_df
}

# 合并新生成的比例样本到原始数据中
if (length(new_ratio_samples) > 0) {
  ratio_combined <- bind_rows(new_ratio_samples)
  plot_data <- bind_rows(plot_data, ratio_combined)
  
  # 更新 SAMPLES_FOR_GENE_BOXPLOT，将新样本名加入列表
  # 这样后续的 Boxplot 和 Heatmap 就会包含这些比例值
  new_names <- unique(ratio_combined$sample)
  SAMPLES_FOR_GENE_BOXPLOT <- c(new_names)
  
  message(paste("--> [完成] 已添加", length(new_names), "个比例样本到数据集。"))
}
...
generate_correlation_matrix_plot <- function(data, feature_type,
  ...
  for (pair in sample_pairs) {
    ...
    plot_df <- plot_df %>% mutate(
      ...
      row = factor(x_name, levels=matrix_samples),
      col = factor(y_name, levels=matrix_samples)
    )
...
sample_pairs <- SPECIFIC_PLOT_PAIRS
...
for (pair in sample_pairs) {
  ...
  # 验证这两个带后缀的样本是否真实存在于合并后的数据中
  available_samples <- unique(plot_data$sample)
  if (!(x_name %in% available_samples) || !(y_name %in% available_samples)) {
      message(paste("    -> 警告: 样本", x_name, "或", y_name, "不存在于合并数据中，跳过此配对。请检查 Slurm 脚本中 SPECIFIC_PLOT_PAIRS 的拼写。"))
      next
  }
...
# --- [核心改动]：获取目标样本名并锁定排序 ---
# intersect(A, B) 会提取 A 和 B 都有的元素，并且严格保留它们在 A 中的原始顺序
if (!exists("SAMPLES_FOR_GENE_BOXPLOT")) {
  target_samples <- unique(plot_data$sample)
} else {
  target_samples <- intersect(SAMPLES_FOR_GENE_BOXPLOT, unique(plot_data$sample))
}

if(length(target_samples) == 0) {
  stop("错误: SAMPLES_FOR_GENE_BOXPLOT 中的样本全都不存在于数据中，跳过绘图。")
}

if(!exists("TE_repname_OI")) TE_repname_OI <- NULL

# --- TE 数据 (过滤掉未指定的样本，并将其转化为固定排序的 Factor) ---
te_data <- plot_data %>%
  filter(featureType=="TE", Class %in% TE_CLASSES_OF_INTEREST, sample %in% target_samples) %>%
  mutate(sample = factor(sample, levels = target_samples)) # 锁定画图顺序
...
# --- Gene 数据 (同样过滤并锁定顺序) ---
gene_data <- plot_data %>%
  filter(featureType=="Gene", sample %in% target_samples) %>%
  mutate(sample = factor(sample, levels = target_samples))

# --- 函数：计算相关性矩阵 (修改以支持严格排序) ---
compute_cor_matrix <- function(df, value_col="lfc_over_igg", ordered_levels=target_samples){
  # 将长数据按 geneID 展开为宽格式，自动对齐不同批次的特征
  wide_df <- df %>%
    select(geneID, sample, all_of(value_col)) %>%
    pivot_wider(names_from = sample, values_from = all_of(value_col)) %>%
    select(-geneID) 
    
  # 强制使宽矩阵的列顺序与你设置的顺序一致
  cols_to_keep <- intersect(ordered_levels, colnames(wide_df))
  wide_df <- wide_df[, cols_to_keep, drop=FALSE]

  # 计算相关性矩阵
  cor_mat <- cor(wide_df, use = "pairwise.complete.obs")

  # 转为长格式，并锁定因子顺序
  # 对 y 轴 (Sample2) 使用 rev() 反转顺序，这样热图的对角线会从左上完美指向右下
  as.data.frame(as.table(cor_mat)) %>%
    setNames(c("Sample1", "Sample2", "Correlation")) %>%
    mutate(
      Sample1 = factor(Sample1, levels = ordered_levels),
      Sample2 = factor(Sample2, levels = rev(ordered_levels)) 
    )
}

# ================= TE 图 =================
# (从这里开始下方的画图代码完全不用动，保持原样即可)
