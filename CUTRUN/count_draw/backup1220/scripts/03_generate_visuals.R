...
if (nrow(te_df_filtered) > 0) {
  ...
  p_family <- ggplot(filtered_te, aes(x = Sample, y = lfc, fill = Sample)) +
    ...
    labs(
      title = paste("TE that highly enriched in ", s1, "and", s3, ", lowly enriched in", s2),
      x = "sample", y = "enrichment value (log2FC vs IgG)", color = "TE element"
    ) +
    theme_bw(base_size = 14) +
  ...
  p_repname <- ggplot(filtered_te, aes(x = Sample, y = lfc, color = Sample)) +
    ...
    labs(
      title = paste("TE that highly enriched in ", s1, "and", s3, ", lowly enriched in", s2),
      subtitle = "Display independently by TE element(repName)",
      x = "Sample", y = "Enrichment value (log2FC vs IgG)"
    ) +
    theme_bw(base_size = 14) +
    theme(
