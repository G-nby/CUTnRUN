...
fruitfly_star_index="/Share/home/gby/datalht/Ensembl_genome/BDGP6/STAR_index_BDGP6"
fruitfly_clean_string="Unmapped|mapped|2110000|rDNA|mitochondrion"
fruitfly_blacklist="/Share/home/gby/datalht/blacklist/dm6-blacklist.v2.bed"
...
if [ $is_trim == "trim" ] && [ ${SEorPE} == "PE" ]; then
        ...
        echo 'Start trimming'
        trim_galore --paired ${file_prefix}*1.*f*q ${file_prefix}*2.*f*q
        echo "Trimming done"
        ...
        STAR --genomeDir ${star_index} --runThreadN 16 --readFilesIn ${file_prefix}_1*_val_1.fq ${file_prefix}_2*_val_2.fq --outFileNamePrefix STAR_mapped_${file_prefix}/${file_prefix}
        STAR --genomeDir ${fruitfly_star_index} --runThreadN 16 --readFilesIn ${file_prefix}_1*_val_1.fq ${file_prefix}_2*_val_2.fq --outFileNamePrefix STAR_spikein_${file_prefix}/spikein_${file_prefix}
        wait
        rm ${file_prefix}*_val_*.fq
elif [ $is_trim == "notrim" ] && [ ${SEorPE} == "PE" ]; then
        echo 'Mode: No trimming + PE'
        #bowtie2 -p 16 -t -q  -N 1 -L 25 -X 700 --no-unal -x ${add_bowtie2_index} -1 ${file_prefix}_1*.f*q -2 ${file_prefix}_2*.f*q > ${file_prefix}.sam 2>>${file_prefix}_log.txt
        STAR --genomeDir ${star_index} --runThreadN 16 --readFilesIn ${file_prefix}_1*.f*q ${file_prefix}_2*.f*q --outFileNamePrefix STAR_mapped_${file_prefix}/${file_prefix}
        STAR --genomeDir ${fruitfly_star_index} --runThreadN 16 --readFilesIn ${file_prefix}_1*.f*q ${file_prefix}_2*.f*q --outFileNamePrefix STAR_spikein_${file_prefix}/spikein_${file_prefix}
        wait
elif [ $is_trim == "trim" ] && [ ${SEorPE} == "SE" ]; then
        # Trimming
        echo 'Mode: Trimming + SE'
        echo 'Start trimming'
        trim_galore ${file_prefix}.f*q
        echo "Trimming done"
        ...
        STAR --genomeDir ${star_index} --runThreadN 16 --readFilesIn ${file_prefix}_trimmed.fq --outFileNamePrefix STAR_mapped_${file_prefix}/${file_prefix}
        STAR --genomeDir ${fruitfly_star_index} --runThreadN 16 --readFilesIn ${file_prefix}_trimmed.fq --outFileNamePrefix STAR_spikein_${file_prefix}/spikein_${file_prefix}
        
        wait
elif [ $is_trim == "notrim" ] && [ ${SEorPE} == "SE" ]; then
        echo 'Mode: No trimming + SE'
        #bowtie2 -p 16 -t -q  -N 1 -L 25 -X 700 --no-unal -x ${add_bowtie2_index} -U ${file_prefix}.f*q > ${file_prefix}.sam 2>>${file_prefix}_log.txt
        STAR --genomeDir ${star_index} --runThreadN 16 --readFilesIn ${file_prefix}.f*q --outFileNamePrefix STAR_mapped_${file_prefix}/${file_prefix}
        STAR --genomeDir ${fruitfly_star_index} --runThreadN 16 --readFilesIn ${file_prefix}.f*q --outFileNamePrefix STAR_spikein_${file_prefix}/spikein_${file_prefix}
        wait
fi
echo "Mapping done"
...
samtools view -F 256 -O bam STAR_mapped_${file_prefix}/${file_prefix}Aligned.out.sam | samtools sort -@ 4 > main_${file_prefix}_prim.bam
samtools view -F 256 -O bam STAR_spikein_${file_prefix}/spikein_${file_prefix}Aligned.out.sam   | samtools sort -@ 4 > spikein_${file_prefix}_prim.bam
wait
...
samtools view -@ 10 -h -b -q 30 -F 1804 -f 2 main_${file_prefix}_prim_rmdup_BLrm.bam > main_${file_prefix}_prim_rmdup_BLrm_Filtered.bam
wait
